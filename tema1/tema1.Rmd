...
* Función que genere 2 números entre 1 y 100 y devuelva su suma
...
Se utilizan para almacenar variables categóricas, con un conjunto fijo de niveles (categorías).
...
Hemos visto que los vectores atómicos agrupan valores individuales (del mismo tipo).
...
---
## 7. Data Frames
Versión bi-dimensional de una lista. Distintas columnas del data frame pueden contener distintos tipos de datos. No obstante, todos los datos de cada columna han de ser del mismo tipo.
...
```{r}
# Seleccionar los fumadores menores de 60
...
Las condicionales y los bucles son las dos estructuras de programación más importantes cualquiera que sea el lenguaje que utilicemos.
...
La generalización de `ifelse` a múltiples condiciones es `case_when` de la librería `dplyr`.
