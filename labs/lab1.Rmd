...
la suma de $n$ números escogidos al azar entre 1 y 100 (con reemplazamiento), dividido entre $n$.
...
La base de datos `mtcars` de R contiene información extraída de la *1974 Motor Trend US magazine* acerca de 10 aspectos de diseño y rendimiento de 32 vehículos. 
...
Para ello:
  ...
  - Crea otra función llamada `compute_gain()` que tenga un argumento `q` referido a la cantidad de dinero de la apuesta, con valor por defecto de 10. Esta función ha de generar una tirada de la ruleta y, en función del resultado, calcular la ganancia y devolverla. Recuerda que
  ...
  - Por último, crea un bucle que simule `K = 10000` partidas de la ruleta, almacena la ganancia generada en cada una, creando así un vector de tamaño $K$. Llama `ganancias` a este vector. Calcula su media. ¿Qué observas?
  
## Ejercicio 6

Utiliza el siguiente código para representar la ganancia acumulada frente al número de partida. Necesitarás haber creado el vector `ganancias` del ejercicio anterior y haber definido el parámetro `K` (número de partidas). Extrae una conclusión del gráfico.
...
2. Calcula el número de puntos que caen dentro del cuarto de círculo. 
