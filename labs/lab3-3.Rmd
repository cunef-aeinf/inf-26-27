---
title: "Lab 3.3 - Strings, factores y fechas"
output:
  html_document:
    css: ./ds_labs.css
    toc: yes
...
y `sched_arr_time` contienen las horas en formato de tres o cuatro dígitos.
Así, 830 son las 8 y media de la mañana y 2030 las ocho y media de la tarde.
Una posibilidad para extraer la hora y los minutos de estas horas es la siguiente:
...
base de datos `airports` y se llama `tzone`.
...
La librería `tidyquant` se puede utilizar para importar a R infinidad de datos económicos. En particular, se puede usar para importar datos sobre el precio
...
Crea una visualización que permita estudiar la variación del precio de 
apertura de la acción de AMZN en el tiempo. ¿En qué fechas se alcanzan precios de 
...
Imagina que hubieses comprado acciones de Amazon el día 2 de marzo de 2020 por
...
Los datos contenidos en `amazon_reviews.csv` contienen reviews y notas que ciertos usuarios han dado a una serie de productos de Amazon. Las variables incluidas son:

* `review`: texto de la review
* `rate`: nota (del 1 al 5)

Crea una nueva variable llamada `n_loves` que corresponda al número de veces que aparece la palabra "love" o "Love" en cada review. Calcula la media de `n_loves` para cada valor de `rate`. Presenta los resultados en un único tibble y extrae una conclusión.
