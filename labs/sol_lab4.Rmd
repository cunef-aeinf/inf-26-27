...
llamadas `lcarat` y `lprice` que correspondan respectivamente a logaritmo en base 
...
Utilizando los datos creados en el ejercicio anterior, ajusta un modelo lineal donde la variable predictora sea `lcarat` y la variable dependiente sea `lprice`. Obtén el valor de los coeficientes e interprétalos. Según este modelo, ¿qué le sucede al precio de un diamante si multiplico por dos su `carat`?
...
ggplot(data = bss_stat, mapping = aes(x = stat)) +
  ...
  geom_vline(xintercept = ci$upper_b,
             color = "steelblue", lty = 2, size = 1) +
  labs(title = "Distribución Bootstrap de la media del tiempo gastado en email",
...
Vamos a utilizar una regresión logística para intentar predecir si los pasajeros del titanic sobrevivieron o no, utilizando como variables predictoras por ejemplo el sexo, la edad, la clase de su ticket, etc.
...
Calcula las predicciones del modelo anterior sobre los datos `test`. Necesitarás añadir type="response" a la función add_predictions. Construye una nueva variable que tome valor 1 si la probabilidad predicha es mayor o igual a 0.5 y cero en caso contrario.

Finalmente, calcula el porcentaje de aciertos comparando la variable recién creada y la variable Survived.
...
Intenta mejorar el modelo anterior añadiendo más variables. ¿Qué variable crees que tiene más poder predictivo?
