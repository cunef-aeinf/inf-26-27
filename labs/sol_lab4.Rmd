---
title: "Lab 3.1 - Importación de datos"
output:
  html_document:
    css: ./ds_labs.css
    toc: yes
...
Lee los datos en un tibble. A continuación, guárdalos en formato csv dentro de una carpeta que se llame `data`.
...
```{r}
ipc %>% filter(observation_date >= "2000-01-01") %>% ggplot(aes(x=observation_date, y=MEDCPIM158SFRBCLE)) + geom_line() + theme_minimal()
```
...
Our World in Data es una iniciativa de la Universidad de Oxford que pretende liberar datos acerca de múltiples fenómenos sociales. En particular ofrece resultados empíricos acerca del cambio de condiciones de vida en el planeta a lo largo del tiempo. Los datos son de diversos ámbitos: económico, social, judicial, ecológico, etc.
...
`"La tasa de desempleo se mantiene por encima del 10%"`
...
if(paro < 10){
  ...
  print("La tasa de desempleo se mantiene por encima del 10%")
}
```
