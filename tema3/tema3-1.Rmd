...
* Aprenderemos cómo descargar y leer datos en R
...
* _na.strings_ - elegir el carácter que representa un valor ausente
...
Podemos indicar qué caracteres se refieren a un valor ausente
...
el primer argumento es el vector a parsear y `na` permite indicar qué caracteres
...
`parse_datetime()` para fecha con hora. Espera, en este orden, año, mes, día, hora, minuto y segundo
```{r}
parse_datetime("2022-01-01T1015")
```

`parse_date()` para fecha. Espera, en este orden, año, mes, día, separados por `-` o `/`
```{r}
...
Para esto, `readr` lee las primeras 1000 filas de cada columna y usa una heurística para adivinar el tipo. Posteriormente, utiliza el parser correspondiente.
...
* `write_excel_csv()`: escribe en excel
...
Recomendable usar `write_rds()` y `read_rds()` (análogas a `readRDS()` y `saveRDS()` de R Base)
...
* Los datos se guardan en objetos con pares atributo-valor y matrices
...
- Procesos **programados** (diarios/semanales), alta **fiabilidad**.
