...
Como parte de la encuesta, se preguntaba a las participantes una vez al año qué habían comido durante los tres días anteriores...
...
Una **variable de confusión** es una variable exógena que es causa tanto de la variable explicativa como de la de respuesta, y que hace que parezca que existe una relación entre ellas.
...
* Estudiaremos modelos que relacionan una serie de variables (variables predictoras)
con una variable de interés (variable respuesta)
...
* Formalmente, un modelo estadístico es un par $(S, \mathcal{P})$: $S$ es el conjunto de posibles observaciones y $\mathcal{P}$ es una familia de distribuciones de probabilidad.
...
El modelo de regresión lineal simple hace, implícitamente, 4 asunciones
...
1. Demostrar que los valores que minimizan la expresión anterior son los estimadores de máxima verosimilitud de $\beta_0$ y $\beta_1$.
...
---
## Interpretación de la regresión lineal - Ordenada en el origen
...
tenemos mayor probabilidad de capturar el valor real del parámetro.
...
Para construir un intervalo de confianza para la media de la población, 
...
variable respuesta relevante (es decir, la variable sobre la que haremos
bootstrap).
...
- `generate()` genera las medias bootstrap
...
- `calculate()` calcula el estadístico muestral. En este caso usamos la media, pero podríamos calcular, por ejemplo,
...
---
## Fundamentos teóricos: intuición 

- Suponemos que nuestros datos vienen de una distribución desconocida $F$.  
- A partir de la muestra observada, construimos la **distribución empírica** $\hat{F}$, que básicamente otorga la misma probabilidad a cada dato observado.  
- El **bootstrap** consiste en tomar muchas muestras con reemplazo de $\hat{F}$, lo que, de forma intuitiva, “simula” volver a muestrear de $F$.  
- Al recalcular el estadístico de interés en cada muestra bootstrap, logramos una aproximación de cómo variaría ese estadístico si realmente volviéramos a tomar muestras de la población original.

---

## Fundamentos teóricos: por qué funciona

- **Idea principal**: En muestras grandes, $\hat{F}$ se acerca cada vez más a $F$.  
  - (Ley de Grandes Números / Consistencia de la distribución empírica).  
- **Consecuencia**: Muestrear de $\hat{F}$ imita suficientemente bien el acto de muestrear de $F$.  
- **Bootstrap**:  
  - Genera la **distribución bootstrap** del estimador, que (bajo condiciones razonables) se aproxima a la distribución real que tendría dicho estimador si muestreásemos repetidamente de la población.  
  - Esta aproximación permite estimar errores estándar, intervalos de confianza, etc., **sin** conocer $F$ ni recurrir a fórmulas complicadas.
...
**Conclusión**: Pese a que estamos “inventando” datos muestreando de $\hat{F}$, resulta que, cuando $n$ es grande, este procedimiento es **estadísticamente confiable** y converge a la misma respuesta que obtendríamos de $F$.
